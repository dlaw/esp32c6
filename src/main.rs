...
mod app {
    ...
    #[local]
    struct Local {
        led: esp_hal::gpio::GpioPin<15>,
        esp_wifi_controller: esp_wifi::EspWifiController<'static>,
    ...
    #[idle(local = [led, esp_wifi_controller, wifi])]
    fn idle(c: idle::Context) -> ! {
        ...
        let (mut wifi_controller, interfaces) =
            esp_wifi::wifi::new(c.local.esp_wifi_controller, c.local.wifi).unwrap();
        ...
        let mut interface = iface::Interface::new(
            iface::Config::new(
                wire::EthernetAddress::from_bytes(&wifi_device.mac_address()).into(),
            ),
            &mut wifi_device,
